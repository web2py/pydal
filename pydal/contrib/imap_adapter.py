...
class IMAPAdapter(NoSQLAdapter):
    ...
    def get_last_message(self, tablename):
        ...
        try:
            ...
            self.db.logger.debug(
                "Error retrieving the last mailbox" + " sequence number. %s" % str(e)
            )
        return last_message
    ...
    def convert_date(self, date, add=None, imf=False):
        ...
        if isinstance(date, basestring):
            ...
            try:
                ...
                self.db.logger.error("Could not parse date text: %s. %s" % (date, e))
                return None
    ...
    def update(self, table, query, fields):
        ...
        if query:
            ...
            store_list = [
                item.strip() for item in data[0].split() if item.strip().isdigit()
            ]
