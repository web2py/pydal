...
class TestMongo(unittest.TestCase):
    ...
    def testVersionCheck(self):
        ...
        with self.assertRaises(Exception):
            db = DAL(
                DEFAULT_URI, attempts=1, check_reserved=["all"], driver_args=driver_args
            )
    ...
    def testRun(self):
        ...
        with self.assertRaises(NotImplementedError):
            db._adapter.select(
                Expression(
                    db, db._adapter.dialect.extract, db.tt.aa, "UNKNOWN", "integer"
                ),
                [db.tt.aa],
                {},
            )
        ...
        db.define_table(
            "tt", Field("aa"), Field("bb", "integer"), Field("cc", "list:integer")
        )
        db.tt.insert(aa="aa")
        ...
        expanded = Expansion(
            db._adapter, "count", Query(db, db._adapter.dialect.eq, db.tt.aa, "x"), []
        )
        ...
        for safe in [False, True, False]:
            ...
            self.assertEqual(
                db._adapter.update(
                    db["tt"],
                    Query(db, db._adapter.dialect.eq, db.tt.aa, "x"),
                    db["tt"]._fields_and_values_for_update({"aa": "x"}).op_values(),
                    safe=safe,
                ),
                0,
            )
...
class TestFields(unittest.TestCase):
    ...
    def testRun(self):
        ...
        insert_vals = [
            ...
            ("time", datetime_datetime_today.time(), datetime_datetime_today.time()),
            ("blob", "x", ""),
            ("blob", b"xyzzy", ""),
...
class TestSelect(unittest.TestCase):
    ...
    def testListReference(self):
        ...
        for ondelete in on_deletes:
            ...
            db.define_table(
                "tt", Field("t0_id", "list:reference t0", ondelete=ondelete)
            )
    ...
    def testGroupByAndDistinct(self):
        ...
        result = db().select(
            db.tt.aa, distinct=db.tt.aa, orderby=~db.tt.aa, limitby=(1, 2)
        )
        self.assertEqual(len(result), 1)
...
class TestBelongs(unittest.TestCase):
    ...
    @unittest.skipIf(
        IS_GAE or IS_MONGODB,
        "Datastore/Mongodb belongs() does not accept nested queries",
    )
    def testNested(self):
        db = self.db
        self.assertEqual(
            db(db.tt.aa.belongs(db(db.tt.id == self.i_id)._select(db.tt.aa))).count(), 1
        )
...
@unittest.skipIf(
    IS_GAE or IS_IMAP, "Contains not supported on GAE Datastore. TODO: IMAP tests"
)
class TestContains(unittest.TestCase):
    def testRun(self):
        ...
        self.assertEqual(
            isinstance(db.tt.insert(aa=["aaa", "bbb"], bb="aaa"), long), True
        )
        self.assertEqual(
            isinstance(db.tt.insert(aa=["bbb", "ddd"], bb="abb"), long), True
        )
        self.assertEqual(
            isinstance(db.tt.insert(aa=["eee", "aaa"], bb="acc"), long), True
        )
        ...
        self.assertEqual(
            isinstance(db.tt.insert(aa=["123", "456"], bb="123", cc=12), long), True
        )
        self.assertEqual(
            isinstance(db.tt.insert(aa=["124", "456"], bb="123", cc=123), long), True
        )
        self.assertEqual(
            isinstance(db.tt.insert(aa=["125", "457"], bb="23", cc=125), long), True
        )
...
class TestDatetime(unittest.TestCase):
    def testRun(self):
        ...
        self.assertEqual(
            isinstance(db.tt.insert(aa=datetime.datetime(1971, 12, 21, 11, 30)), long),
            True,
        )
        self.assertEqual(
            isinstance(db.tt.insert(aa=datetime.datetime(1971, 11, 21, 10, 30)), long),
            True,
        )
        self.assertEqual(
            isinstance(db.tt.insert(aa=datetime.datetime(1970, 12, 21, 9, 31)), long),
            True,
        )
        self.assertEqual(
...
class TestExpressions(unittest.TestCase):
    def testRun(self):
        ...
        for dal_opt in DAL_OPTS:
            ...
            # test update
            self.assertEqual(
                db(db.tt.aa == 3).update(aa=db.tt.aa + 1, bb=db.tt.bb + 2), 1
            )
            ...
            self.assertEqual(db(db.tt.aa == 6).count(), 1)
            self.assertEqual(
                db(db.tt.aa == 6).update(aa=db.tt.aa % 4 + 1, cc=db.tt.cc + "1" + "1"),
                1,
            )
...
@unittest.skipIf(
    IS_GAE or IS_IMAP,
    'TODO: Datastore throws "AttributeError: Row object has no attribute _extra"',
)
    ...
    def testRun(self):
        ...
        try:
            ...
            if sys.version < "2.6.5":
                unicode_keys = False
            db3 = DAL(**serializers.loads_json(dbjson, unicode_keys=unicode_keys))
            assert (
                hasattr(db3, "person")
                and hasattr(db3.person, "uuid")
                and db3.person.uuid.type == db.person.uuid.type
            )
            drop(db3.person)
            db3.commit()
            db3.close()
        ...
        assert (
            isinstance(db4.tvshow.insert(), long),
            isinstance(db4.tvshow.insert(name="Loriot"), long),
            isinstance(db4.tvshow.insert(name="Il Mattatore"), long),
        ) == (True, True, True)
...
class TestSelectAsDict(unittest.TestCase):
    def testSelect(self):
        ...
        rtn = (
            db(db.a_table)
            .select(db.a_table.id, db.a_table.b_field, db.a_table.a_field)
            .as_list()
        )
        ...
        self.assertEqual(
            ("id" in keys, "b_field" in keys, "a_field" in keys), (True, True, True)
        )
        drop(db.a_table)
        db.close()
...
class TestRNameFields(unittest.TestCase):
    ...
    def testSelect(self):
        ...
        db.define_table(
            "person", Field("name", default="Michael", rname=rname), Field("uuid")
        )
    ...
    @unittest.skipIf(
        IS_GAE, "TODO: Datastore does not accept dict objects as json field input."
    )
    def testRun(self):
        db = DAL(DEFAULT_URI, check_reserved=["all"])
        rname = _quote(db, "a very complicated fieldname")
...
class TestQuoting(unittest.TestCase):
    ...
    def testCase(self):
        ...
        # test field case
        t0 = db.define_table("table is a test", Field("a_a"), Field("a_A"))
...
class TestRecordVersioning(unittest.TestCase):
    def testRun(self):
        db = DAL(DEFAULT_URI, check_reserved=["all"])
        tt = db.define_table(
            "tt", Field("name"), Field("is_active", "boolean", default=True)
        )
